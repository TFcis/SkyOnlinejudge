...
<div id="wrap"> 
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        <a class="navbar-brand" href="<?=$_E['SITEROOT']?>index.php"><?=\SKYOJ\html($_E['site']['name'])?></a>
        ...
        <div class="collapse navbar-collapse" id="main_nav">
            <ul class="navbar-nav mr-auto">
                <li class="nav-item"><a class="nav-link" href="<?=$SkyOJ->uri('problem','list')?>">Problems</a></li>
                <li class="nav-item"><a class="nav-link" href="<?=$SkyOJ->uri('chal')?>">Challenges</a></li>
                <li class="nav-item"><a class="nav-link" href="<?=$SkyOJ->uri('contest','list')?>">Contest</a></li>
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu" aria-labelledby="main_nav_dropdown">
                        <a style="color:#000" class="dropdown-item" href="<?=$SkyOJ->uri('rank','list')?>">Scoreboard</a>
                        <a style="color:#000" class="dropdown-item" href="<?=$SkyOJ->uri('code')?>">Code</a>
                        <div class="dropdown-divider"></div>
                        <a style="color:#000" class="dropdown-item" href="<?=$SkyOJ->uri('index','old')?>">Dev Message</a>
                    </div>
                </li>
            </ul>
            <ul class="navbar-nav ml-auto">
                <?php if (!$_G['uid']): ?>
                    <li class="nav-item"><a class="nav-link" href="<?=$SkyOJ->uri('user','login')?>">LOGIN</a></li>
                <?php else: ?>
                    <?php if( userControl::isAdmin($_G['uid']) ):?>
                        <li class="nav-item"><a class="nav-link" href="#" id="admin-tab">Admin</a></li>
                    <?php endif; ?>
                    <li class="nav-item"><a class="nav-link" href="<?=$SkyOJ->uri('user','view',$_G['uid'])?>"><?=\SKYOJ\html($_G['nickname']); ?></a></li>
                    <li class="nav-item"><a class="nav-link" href="<?=$SkyOJ->uri('user','logout')?>">LOGOUT</a></li>
                <?php endif; ?>
            </ul>
        </div>
    </nav>
    ...
    <?php if(false && userControl::isAdmin($_G['uid'])) :/*Admin Pass input*/?>
